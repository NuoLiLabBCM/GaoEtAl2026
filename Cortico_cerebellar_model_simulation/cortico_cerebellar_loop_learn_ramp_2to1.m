%% cortico_cerebellar_loop_learn_ramp_2to1.m
...
%
% Training:
%   Stage 1: set up initial network to produce a 0-to-1 ramp over 2 s
%   Stage 2: training the GC->PC weights to produce a 0-to-1 ramp over 1 s
%
% Notes:
%   - Only GC->PC weights are trained.
%   - Stage 2 training only uses LTD at GC->PC weights
%   - Stage 2 still simulates to 2 s, but the loss is computed only from 0-1 s.
%
...
% Training parameters.
nEpochStage1 = 400;   % train to 2-s ramp
nEpochStage2 = 400;   % adapt to 1-s ramp
...
%% ---------------- Stage 1: train 2-s ramp ----------------
fprintf('Training stage 1: 2-s ramp target...\n');
% This is the initial training stage for setting up the network to have ramp
% no constrains are imposed for network weights
...
%% ---------------- Stage 2: retrain/adapt to 1-s ramp ----------------
% **This is the part of training that capture within-session delay learning
% The weight change for stage 2 learning is contrained to be negative, to
% model LTD only
fprintf('Training stage 2: 1-s ramp target, LTD only...\n');

% Loss weights for Stage 2
% Simulate the full 2 s trajectory, but compute loss only from 0 to 1 s.

lossWeight1s = single(p.t <= 1.0);

% Normalize so weighted loss scale is comparable
...
[r2, gc2, pc2] = forwardModel(dlarray(thetaStage1), p);
[r1, gc1, pc1] = forwardModel(theta, p);
...
title('Readout Stage 1: setting up initial network to ramp');
legend(['Target', compose('epoch %d', snapEpochs1)], 'Location','southeast');
box off;
...
plot(p.t, extractdata(target2s), '--', 'color',[.7 .7 .7], 'LineWidth', 2);
...
xlabel('Time (s)');
ylabel('Readout activity');
title('Readout Stage 2: adapting to 1-s ramp');
ylim([0 1])
xlim([0 2])
xline(1.0, 'k:');
...
title('GC activity at 2-s delay (before learning)');
colorbar;
...
xlabel('Time (s)');
ylabel('GC time constant \tau (s)');
title('GC activity after learning 1-s delay');
colorbar;
formatTauYAxis(sortedTau, p.Ngc);
...
xlabel('Time (s)');
ylabel('PC activity');
title('PCs at 2-s delay (before learning)');
...
xlabel('Time (s)');
ylabel('PC activity');
title('PCs after learning 1-s delay');
legend('PC1','PC2','Location','best');
box off;
...
legend('After 2-s learning', 'After 1-s learning', 'Location','best');
box off;
xlim([1, 2*p.Ngc]);
...
title('Stage 2 weight change: 2-s ramp to 1-s ramp');
box off;
xlim([1, 2*p.Ngc]);
