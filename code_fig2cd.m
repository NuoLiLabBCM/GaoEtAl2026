...
proj_activity = [];
n = 1;
for i_sess = 1 : size(proj_ctrl_allsession,1)
    trials = trl_info_allsession{i_sess,1}{1,1};
    proj_norm = proj_ctrl_allsession{i_sess,1}{1,1}; % 1 is alm ,2 is th
    if ~isempty(trials)
    proj_activity(n,:,:,:,1) = squeeze(mean(proj_norm(trials.yes_early_test & trials.R_hit,:,:,:),1));    %(sess,t,cd/ramp,early/late,trl_type)            
    proj_activity(n,:,:,:,2) = squeeze(mean(proj_norm(trials.no_early_test & trials.L_hit,:,:,:),1));                
    proj_activity(n,:,:,:,3) = squeeze(mean(proj_norm(trials.yes_late_test & trials.R_hit,:,:,:),1));                
    proj_activity(n,:,:,:,4) = squeeze(mean(proj_norm(trials.no_late_test & trials.L_hit,:,:,:),1));                
