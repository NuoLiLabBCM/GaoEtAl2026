...
x_line = x;
y_line = mean(y,1,'omitmissing');

if (nargin == 6 & sem_option == 1)
    y_sem = std(y,'omitmissing')/sqrt(size(y,1));
...
elseif (nargin == 6 & sem_option == 3)
    ...
    for i=1:1000
        ...
        y_tmp(i,:) = mean(y_isample,1,'omitmissing');
    end
    y_sem = std(y_tmp);
elseif (nargin == 7)
    y_sem = std(y,[],1)*n_std;
...
if nargin >= 5
    ...
    alpha(0.5)
    plot(x_line, y_line, 'color', line_color, 'linewidth', 1); hold on
end
