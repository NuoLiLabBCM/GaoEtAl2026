function [orthonormal_basis var_allDim] = func_compute_activity_modes_DRT_partial(PSTH_yes_correct, PSTH_no_correct, PSTH_yes_error, PSTH_no_error, T_cue_aligned_sel, time_epochs)
...
orthonormal_basis = Gram_Schmidt_process_partial([CD_stim_mode CD_choice_mode CD_outcome_mode CD_sample_mode CD_delay_mode CD_go_mode Ramping_mode GoDirction_mode v],16);

proj_allDim = activityRL'*orthonormal_basis;

% restrict response mode to be positive
i_t3 = find(T_cue_aligned_sel(1,:)>(t_response+.1) & T_cue_aligned_sel(1,:)<(t_response+0.5));
if sum(proj_allDim(i_t3,9)) < 0
    orthonormal_basis(:,9) = - orthonormal_basis(:,9);
end
