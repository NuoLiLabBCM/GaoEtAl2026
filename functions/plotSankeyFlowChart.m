...
%%%%
try 
    layer_cat_names = options.layer_cat_names;
catch
end

try
    layer_point_names = options.layer_point_names;
catch
end
%%%%%
...
for t = 1:n_layers
    if istable(data)
        ...
        % runbo added
        %%%%%
        if ~exist('layer_point_names','var')
    ...
    for c = 1:max_n_cats_layer
        ...
        if show_perc
            if val > 0
                ...
                text(X(t), bars{t}(c)/2 + sum(bars{t}(1:c-1)) + (c-1)*gap, txt2print,'HorizontalAlignment','center','Color',text_color,'Interpreter','None')
            end
        else
            if show_cat_labels
                if val > 0
                    text(X(t), bars{t}(c)/2 + sum(bars{t}(1:c-1)) + (c-1)*gap, layer_cat_names{t}{c},'HorizontalAlignment','center','Color',text_color,'Interpreter','None')
                end
            end
        end
    end
